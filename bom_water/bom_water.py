...
class Action:
    ...
    def GetDataAvailability(self):
        return "http://www.bom.gov.au/waterdata/services?service=SOS&version=2.0&request=GetDataAvailability"

    @property
    def GetObservation(self):
        return "http://www.opengis.net/def/serviceOperation/sos/core/2.0/GetObservation"

    @property
    def GetFeatureOfInterest(self):
        return "http://www.opengis.net/def/serviceOperation/sos/foiRetrieval/2.0/GetFeatureOfInterest"
...
class BomWater:
    ...
    def check_cache_status(self):
        if os.path.exists(self.waterML_GetCapabilities):
            ...
            print(
                f"one time creating cache directory and files, this will take a little time please wait."
            )
    ...
    def init_properties(self):
        ...
        """Properties"""
        properties = getCap_json["sos:Capabilities"]["sos:contents"]["sos:Contents"][
        ...
        """Procedures"""
        offerings = getCap_json["sos:Capabilities"]["sos:contents"]["sos:Contents"][
            "swes:offering"
        ]
        ...
        for index in range(len(getfeature_json["features"])):
            long_statioId = getfeature_json["features"][index]["properties"][
                "long_name"
            ]
    ...
    def body(
        ...
        lower_corner: Optional[float],
        upper_corner: Optional[float],
    ):
    ...
    def spatial(self, action: str, lower_corner: float, upper_corner: float):
        ...
        envelope = (
            f'<gml:Envelope srsName="http://www.opengis.net/def/crs/EPSG/0/4326">'
        )
    ...
    def build_payload(
        ...
        return (
            ...
            + self.body(
                action, feature, prop, proced, begin, end, lower_corner, upper_corner
            )
            + self.envelope_tail()
        )
    ...
    def _parse_default_node(self, default_node):
        default_props = {"quality_code": -1, "unit": "", "interpolation": ""}
    ...
    def parse_get_data(self, response, raw=False):
        ...
        for e in root.findall(query_measurement):
            ...
            for node in e:
                if node.tag.endswith("time"):
                    ...
                    qcodes = self._parse_quality_code(node, qdefault, idefault)
                    info[2] = qcodes[0]
                    info[3] = qcodes[1]
            data.append(info)
        ...
        df = pd.DataFrame(
            data,
            columns=("Timestamp[UTC]", f"Value[{unit}]", "Quality", "Interpolation"),
        )
    ...
    def create_feature_list(self, bom_response_foi:Dict[str,Any], path):
        ...
        su = spatail_utilty()
        features = bom_response_foi["soap12:Envelope"]["soap12:Body"][
        ...
        for feat in features:
            ...
            if (
                "#text"
                in feat["wml2:MonitoringPoint"]["sams:shape"]["gml:Point"]["gml:pos"]
            ):
                pos = feat["wml2:MonitoringPoint"]["sams:shape"]["gml:Point"][
                    "gml:pos"
                ]["#text"]
            else:
                lat = ""
            ...
            name = (
                feat["wml2:MonitoringPoint"]["gml:name"]
                .replace(" ", "_")
                .replace("-", "_")
            )
